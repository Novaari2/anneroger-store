...
export default function TransactionBlock({transaction,  setOpen, setDetails}) {
    function setStatus(status, transaction) {
        router.put(route('transactions.update', [transaction]), {
            order_status: status,
        }, {
            preserveScroll: true,
            preserveState: true,
    ...
    return (
        <div className='overflow-hidden flex flex-col h-[35rem] rounded-lg bg-white p-4 text-sm shadow'>
            <div className='flex-1'>
                <div className='mb-5 flex justify-between'>
                    ...
                    {transaction.order_status === 4 && (
                        <IconHomeCheck className='text-emerald-500 stroke-[1.25]'/>
                    )}
                </div>
                <div className='mb-5'>
                    <span>Address:</span>
            ...
            <div>
                <button className='px-2 py-1 border bg-gray-50 text-sm font-medium rounded' onClick={() => {
